export default function ({ db, express, bcrypt, jwt, jwtToken }) {
  ...
  routes.post('/google', (req, res) => {
    if (!req.body.token)
      return res.status(500).json({ type: 'error', message: 'No access token provided.' });
    ...
    plus.people.get({
      ...
      if (error)
        return res.status(500).json({ type: 'error', message: 'plus people get error', error });
      const emails = (response.data || {}).emails;
      if (!emails || emails.length == 0)
        return res.status(500).json({ type: 'error', message: 'User email not found in google+.' });
      db.query('select * from `users` where `email`=?', emails[0].value, (error, results) => {
        if (error)
          return res.status(500).json({ type: 'error', message: 'db error', error });
        if (results.length == 0)
          return res.status(403).json({ type: 'error', message: 'User with email from google+ not found in database.' });
        const user = results[0];
        return res.json({
          type: 'success',
